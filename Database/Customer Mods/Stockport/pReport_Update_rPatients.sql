...
--25Apr14 TH  Use different alias for Stockport Case number (TFS 86070)


DECLARE @TEXT 				VARCHAR (2500)
DECLARE @LiveDB			VARCHAR (50)
...
FROM ' + @LiveDB + '.icwsys.EntityAlias A
INNER JOIN ' + @LiveDB + '.icwsys.AliasGroup B ON A.AliasGroupID = B.AliasGroupID
WHERE B.[Description] = ''HospitalCaseNoteNumber'' AND LEN(ISNULL(A.Alias, '''')) > 0 AND A.[Default] = ''1'''
...
LEFT JOIN (Select F.entityid, G.[BoxNumber], G.[DoorNumber], G.[Building], G.[Street],G.[Town],
	G.[LocalAuthority], G.[District], G.[PostCode], G.[Province], G.[Country]
 from  ' + @LiveDB + '.icwsys.entitylinkaddress  F
		JOIN ' + @LiveDB + '.icwsys.address G on G.addressid = F.addressid
		JOIN ' + @LiveDB + '.icwsys.addresstype H on F.addresstypeid = H.addresstypeid
		JOIN ##DistinctAdd da on da.AddressID = F.AddressID
		where H.description  = ''' + @AddressType + ''') I on I.entityID = A.entityID'
EXECUTE (@TEXT)
