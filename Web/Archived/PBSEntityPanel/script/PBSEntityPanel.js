function window_onload()
{
	...
	ICWToolMenuEnable("PBSPatient_Edit", blnEpisodeIsSelected);
...
function LaunchEditPBSPatient(lngEpisodeID)
{
	if ( lngEpisodeID == undefined ) lngEpisodeID = -1;
	//lngEpisodeID = window.showModalDialog("../PatientEditor/PatientEditModal.aspx?SessionID=" + document.all("bdy").getAttribute("SessionID") + "&EpisodeID=" + lngEpisodeID ,"","dialogHeight: 610px; dialogWidth: 420px; edge: Raised; center: Yes; Scroll: No; help: No; resizable: No; status: No;");
//TH THIS SHOULD BE USED AS IS I THINK	
	lngEpisodeID = window.showModalDialog("../PBSPatientEditor/PBSPatientEditorModal.aspx?SessionID=" + document.all("bdy").getAttribute("SessionID") +"&EpisodeID=-1&Mode=editpatient&WindowStyle=Modal","","dialogHeight: 600px; dialogWidth: 800px; edge: Raised; center: Yes; Scroll: No; help: No; resizable: No; status: No;");
	if (lngEpisodeID == 'logoutFromActivityTimeout') {
		lngEpisodeID = null;
		window.close();
		window.parent.close();
		window.parent.ICWWindow().Exit();
	}
	if (lngEpisodeID!=nul && lngEpisodeID > -1)
	{
...
function LaunchPBSFastRepeatSearch(lngSessionID)
{
	...
	if (lngEpisodeID!=null && lngEpisodeID > -1)
	{
	    // 21Feb11 PH Take ICW Episode integer, convert to entity & episode versioned identifiers, and raise the ICW Episode Selected Event
	    // Create JSON episode event data
	    var jsonEntityEpisodeVid = ICW.clinical.episode.eventSelectedRaised(lngEpisodeID, 0, document.all("bdy").getAttribute("SessionID"));
