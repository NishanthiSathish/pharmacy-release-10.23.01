...
function body_onload()
{
    ...
    DisplayPharmacySupplierList('', '', sortBy);
}
...
function btnAdd_onclick()
{
    ...
    var result = window.showModalDialog('NewPharmacySupplierWizard.aspx' + getURLParameters(), '', 'status:off; center:Yes;');
    ...
    if (result != undefined)
        __doPostBack('upMain', 'LoadItem:true:' + result);
}
...
function btnEdit_onclick()
{
    ...
    DisplayPharmacySupplierList($('#hfSelectedCode').val(), '', sortBy);
}

// Call to allow user to select a pharmacy ward
function DisplayPharmacySupplierList(defaultCode, optionalRow, sortBy)
{
    ...
    var result = window.showModalDialog('../PharmacySupplierEditor/SelectPharmacySupplier.aspx' + strURLParameters, '', 'status:off; center:Yes;');
    if (result == 'logoutFromActivityTimeout') {
        result = null;
        window.close();
        window.parent.close();
    ...
    if (result != undefined && result.split('|').length > 1)
    {
        ...
        if (code == optionalRow)
            ...
            __doPostBack('upMain', 'LoadItem:false:' + code);
    }            
}
...
// Displays the change report for the current supplier
// 18May15 XN 117528
function ChangeReport_onclick()
{
    var wsupplierID = $('#hfWSupplierID').val();
    if (wsupplierID != '')
    {
        var strURLParameters = getURLParameters();
        ...
        if (selectedSiteID != undefined)
            ReportPharmacyLogWSupplier2(wsupplierID, undefined, false);
    }
}