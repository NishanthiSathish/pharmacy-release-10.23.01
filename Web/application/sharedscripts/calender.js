/*
TH 20Jan03 Used script from source below and made alterations to  
allow a date to be input to the picker (uses now if no date       
supplied).At present date input must be in dd/mm/yyyy format.     
assumes date is valid. Do validation before launching picker with 
incoming date.													   
Also changed appearance and removed print option                  

To use this date picker look at the following e.g.                
This if from the body of the document                             
...
function show_calendar() {
	...
	*/
	p_item = arguments[0];
	var strTextArea = arguments[1];
	strEndDate = new String(document.getElementById(strTextArea).value);
	if ((strEndDate == null) || (strEndDate.length > 10) || (strEndDate.length < 10)){
	}
	else
	{
		var intDay = eval(strEndDate.substring(0,2));
		var intMonth = eval(strEndDate.substring(3,5));
		var intyear = eval(strEndDate.substring(6,10));
		gNow.setDate(intDay);
		gNow.setMonth(intMonth - 1);
		gNow.setYear(intyear);
	}	
	if (arguments[2] == null)
