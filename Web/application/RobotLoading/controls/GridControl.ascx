<%@ Control Language="C#" AutoEventWireup="true" CodeFile="GridControl.ascx.cs" Inherits="application_RobotLoading_controls_GridControl" %>
    ...
    <%
            for (int r = 0; r < rows.Count; r++) 
            {
                ...
                if (!string.IsNullOrEmpty(JavaEventClick))
                    attributes.Append(string.Format("onclick=\"gridcontrol_onclick_internal('{0}',{2});{1}({2})\" ", controlID, JavaEventClick, r));
                else
                    attributes.Append(string.Format("onclick=\"gridcontrol_onclick_internal('{0}',{1})\" ", controlID, r));
