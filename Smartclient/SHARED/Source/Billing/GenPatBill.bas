...
'21Nov13 TH billpatient: Entry to generic billing (TFS 77893)
...
Dim blnNoRpts As Boolean
   ...
   '21Nov13 TH Entry to generic billing (TFS 77893)
   If Val(TxtD(dispdata$ & "\Patbill.ini", "PatientBilling", "", "BillingType", 0)) = 4 Then
...
Public Function BillPatDispensQty(stockLevel!, DispensQty!, negorpos%, escd%) As Integer

   If Val(TxtD(dispdata$ & "\Patbill.ini", "PatientBilling", "", "BillingType", 0)) = 4 Then
      BillPatDispensQty = Bmi_BillPatDispensQty(stockLevel!, DispensQty!, negorpos%, escd%)
   End If
