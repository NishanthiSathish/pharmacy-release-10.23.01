...
'02Sep10 TH Added DeliveryNoteReference field for UMMC FINV. This is the docket number from the external receipt docket. (F0054531)
'02Sep10 TH PutOrder: Added DeliveryNoteReference (UMMC FINV) (F0054531)
'02Sep10 TH FillordfromRS: Added DeliveryNoteReference for Delivery note capture (UMMC FINV) (F0054531)
'15Oct10 XN Updated DeliveryNoteReference length to 10 to match db (F0098792) (UMMC FINV)
'19Oct10 XN Updated DeliveryNoteReference length to 20 (F0098792) (UMMC FINV)
'04Aug15 TH GetOrderDLOWard: Added check - we dont return DLO unless its a genuine order number (TFS 124853)
...
'16Aug16 XN FillOrdFromRS: Fixed if WWardProductListLineID is null 160233 
...
Type orderstruct
   ...
   WWardProductListLineID As Long          '16Dec14 TH Added for Wardstocklist overhaul
   EDIProductIdentifier AS String *15 	'22Jul16 XN Added 126634
End Type                         'total length is now 1024 bytes

Private Const OBJNAME As String = PROJECT & "WOrderIO."
...
''         'OPEN TRANSACTION
''         blnOK = False
''         gTransport.Connection.BeginTrans
''         Do While Not blnOK
''            On Error Resume Next
''            blnOK = gTransport.GetRowLock(g_SessionID, strTable, lngOrderID) '21Oct04 TH Testage
''            On Error GoTo 0
''            If Not blnOK Then popmessagecr "", "Record Locked. Please Try Again"
''         Loop
''      End If
      If blnOK Then
         ...
         If blnLockRecord Then 'Stop            '!!** Need locking hint here
            ...
            If TrueFalse(TxtD(dispdata$ & "\ascribe.ini", "", "Y", "SessionLocking", 0)) Then
               ...
               Do While Not blnOK
                  ...
                  If rsLock.EOF Or rsLock.RecordCount > 1 Then
                     Do While gTransportIsInTransaction(g_SessionID)      '15Aug12 CKJ
                        'Here are going to rollback ay outstanding transactions prior to msg display
                        'We keep a count so we can reinstitute them
                        If gTransportIsInTransaction(g_SessionID) Then gTransportConnectionExecute "Rollback Transaction"      '15Aug12 CKJ
                        intCount = intCount + 1
                     Loop
            ...
            Else
               ...
               Do While Not blnOK
                  ...
                  blnOK = gTransport.GetRowLock(g_SessionID, strTable, lngOrderID) '21Oct04 TH Testage
                  If Not blnOK Then
                     gTransportConnectionExecute "RollBack Transaction"       '06Jan06 TH Moved from below msgbox call      '15Aug12 CKJ
...
'02Sep10 TH Added DeliveryNoteReference for Delivery note capture (UMMC FINV) (F0054531)
'16Aug16 XN Fixed if WWardProductListLineID is null 160233
...
If Not rsOrder.EOF Then
   ...
   With ord
	  ...
	  '.WWardProductListLineID =rsOrder.Fields("WWardProductListLineID")	16Aug16 XN Fixed if WWardProductListLineID is null 160233  '16Dec14 TH Added
      .WWardProductListLineID = GetField(rsOrder.Fields("WWardProductListLineID")) 
...
'V93 redone for SQL
'01Nov05 TH Extended invnum
'16Jan07 TH Altered reconciledate and added codingslipdate as these were not being sent to DB
'02Sep10 TH Added DeliveryNoteReference (UMMC FINV) (F0054531)
'15Oct10 XN Updated DeliveryNoteReference length to 10 to match db (F0098792) (UMMC FINV)
'19Oct10 XN Updated DeliveryNoteReference length to 20 (F0098792) (UMMC FINV)
'15Aug11 TH Increased DeliveryNoteReference length to 30 for Notts URS F0084761
'29May12 TH Added DLO Fields
...
Const ErrSource As String = "PutOrder"
   ...
   If TrueFalse(TxtD(dispdata$ & "\ascribe.ini", "", "Y", "SessionLocking", 0)) And lngOrderID > 0 Then '03Jan06 TH Added check on lngOrderID as we wont be in transaction if this is new rec
      If Not TrueFalse(TxtD(dispdata$ & "\ascribe.ini", "", "N", "SessionLockingStandard", 0)) Then gTransportConnectionExecute "begin Transaction"      '15Aug12 CKJ
   End If
   ...
   With ord
      ...
      strSql = strSql & gTransport.CreateInputParameterXML("DLO", trnDataTypeBit, 1, .DLO) & _
                        gTransport.CreateInputParameterXML("DLOWard", trnDataTypeVarChar, 5, .DLOWard)
      '04Sep12 TH PSO (only on Ordering and reconciliation
      If LCase$(strTable) <> "wrequis" Then
         strSql = strSql & gTransport.CreateInputParameterXML("PSORequestID", trnDataTypeint, 4, .PSORequestID)
		 strSql = strSql & gTransport.CreateInputParameterXML("EDIProductIdentifier", trnDataTypeVarChar, 15, .EDIProductIdentifier)	' 22Jul16 XN added 126634 
      Else '16Dec14 TH Record the originating ward stock list line (if applicable)
         'strSql = strSql & gTransport.CreateInputParameterXML("WWardProductListItemID", trnDataTypeint, 4, .PSORequestID)
         strSql = strSql & gTransport.CreateInputParameterXML("WWardProductListLineID", trnDataTypeint, 4, .WWardProductListLineID) '07Jan14 TH Use proper field (TFS 102828)
      End If
   ...
   If lngOrderID = 0 Then
      'gTransport.Connection.Execute "begin Transaction"      '15Aug12 CKJ
      lngReturn = gTransport.ExecuteInsertSP(g_SessionID, strTable, strSql)
      'gTransport.Connection.Execute "Commit Transaction"      '15Aug12 CKJ
...
Cleanup:
   ...
   Else
      ...
      gTransportConnectionExecute "RollBack Transaction"      '15Aug12 CKJ
      End If
   End If
   On Error Resume Next
   '15Aug12 CKJ gTransport.Connection.RollbackTrans replaced with...
   gTransportConnectionExecute "RollBack Transaction"      '15Aug12 CKJ
   PutOrder = lngReturn
   On Error GoTo 0
   If ErrNumber Then Err.Raise ErrNumber, OBJNAME & ErrSource, ErrDescription
...
Public Function CalculateWOrderValueDLO(ByVal strSupplierCode As String) As String
'08Jul12 TH Written to allow for "stratified" value based on DLO Wards
'15 Added Direct Location to DLO order line descriptor
Dim strParams As String
Dim rsOrders As ADODB.Recordset
Dim totalcost As Double
Dim cost As Double
Dim strDLOWard As String
Dim strTemp As String
Dim ErrNumber As Long, ErrDescription As String
Const ErrSource As String = "CalculateWOrderValueDLO"
   
   On Error GoTo CalculateWOrderValueDLOErr
   
   strParams = gTransport.CreateInputParameterXML("SiteID", trnDataTypeint, 4, gDispSite) & _
   gTransport.CreateInputParameterXML("SupCode", trnDataTypeVarChar, 5, strSupplierCode)
   
   Set rsOrders = gTransport.ExecuteSelectSP(g_SessionID, "pWOrderValueDLO", strParams)
   ...
   If Not rsOrders Is Nothing Then     'use returned recordset
      If rsOrders.State = adStateOpen Then
         If rsOrders.RecordCount <> 0 Then
            rsOrders.MoveFirst
            strDLOWard = Trim$(UCase$(RtrimGetField(rsOrders!DLOWard)))  'set for first go
            Do While Not rsOrders.EOF
               If Trim$(UCase$(RtrimGetField(rsOrders!DLOWard))) <> strDLOWard Then
                  strTemp = strTemp & Iff((Trim$(strTemp) = ""), crlf, crlf & crlf) & Iff((Trim$(strDLOWard) = ""), "General Order : ", "DIrect Location Order For : " & strDLOWard & crlf & crlf) & "Total cost for this order is  " & money(5) & " " & Format$(totalcost / 100, "0.00")
                  strDLOWard = Trim$(UCase$(RtrimGetField(rsOrders!DLOWard)))
                  totalcost = 0
               End If
               ...
               rsOrders.MoveNext
            Loop
            strTemp = strTemp & Iff((Trim$(strTemp) = ""), crlf, crlf & crlf) & Iff((Trim$(strDLOWard) = ""), Iff((Trim$(strTemp) = ""), "", "General Order : " & crlf & crlf), strDLOWard & crlf & crlf) & "Total cost for this order is  " & money(5) & " " & Format$(totalcost / 100, "0.00")
            totalcost = 0
         End If
      End If
   End If
   CalculateWOrderValueDLO = strTemp
...
CalculateWOrderValueDLOErr:
   ErrNumber = Err.Number
   ErrDescription = Err.Description
Resume Cleanup
End Function
Public Function InvoicableOrder(ByVal lngOrderNum As Long, ByVal blnInvoice As Boolean, ByVal blnPSO As Boolean) As Boolean
'14Nov05 TH Small wrapper to check if a given order line exists
'03Sep12 TH Mod to support PSO
...
Dim rsOrders As ADODB.Recordset
   ...
   If blnInvoice Then
      Set rsOrders = gTransport.ExecuteSelectSP(g_SessionID, "pWReconcilbyCriteriaINVOICESlimline" & Iff(blnPSO, "PSO", ""), strParams)
   Else
...
Public Function OrderIncludesBatchTrackItems(ByVal lngOrderNum As Long) As Boolean
'31May TH () Small wrapper to check if a given order on receipt has batch tracking for any of the items on it
Dim strParams As String
Dim lngResult As Long

   OrderIncludesBatchTrackItems = False

   strParams = gTransport.CreateInputParameterXML("SiteID", trnDataTypeint, 4, gDispSite) & _
               gTransport.CreateInputParameterXML("num", trnDataTypeint, 4, lngOrderNum)
   lngResult = gTransport.ExecuteSelectReturnSP(g_SessionID, "pWOrderCountToReceivewithBatchItemsbyNum", strParams)
   If lngResult > 0 Then OrderIncludesBatchTrackItems = True
   
End Function

Public Function GetOrderDLOWard(ByVal lngOrderNum As Long) As String
'04Aug15 TH Added check - we dont return DLO unless its a genuine order number (TFS 124853)
Dim strParams As String
Dim rsOrders As ADODB.Recordset
Dim strResult As String

   strResult = ""
   If lngOrderNum > 0 Then    '04Aug15 TH Added - we dont return DLO unless its a genuine order number (TFS 124853)
      strParams = gTransport.CreateInputParameterXML("SiteID", trnDataTypeint, 4, gDispSite) & _
                  gTransport.CreateInputParameterXML("Ordernum", trnDataTypeint, 4, lngOrderNum)
                  
      Set rsOrders = gTransport.ExecuteSelectSP(g_SessionID, "pWOrderDLObySiteandOrderNum", strParams)
      
      If Not rsOrders.EOF Then
         rsOrders.MoveFirst
         strResult = RtrimGetField(rsOrders!DLOWard)
      ...
      Set rsOrders = Nothing
   End If
   GetOrderDLOWard = strResult
   
End Function

Public Function GetPickingTicketDLO(ByVal lngPickNum As Long) As Boolean
Dim strParams As String
Dim rsOrders As ADODB.Recordset
Dim strResult As Boolean
   ...
   strParams = gTransport.CreateInputParameterXML("SiteID", trnDataTypeint, 4, gDispSite) & _
               gTransport.CreateInputParameterXML("PickTicknum", trnDataTypeint, 4, lngPickNum)
               
   Set rsOrders = gTransport.ExecuteSelectSP(g_SessionID, "pWRequisDLObySiteandPickTickNum", strParams)
   
   If Not rsOrders.EOF Then
      rsOrders.MoveFirst
      strResult = RtrimGetField(rsOrders!DLO)
