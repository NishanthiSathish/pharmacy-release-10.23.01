...
'            ProcessPN: added handling of regimen requestID
'
'06Nov14 XN  RefreshState: Added setting BSA (83897)
'04Nov15 TH  ProcessPN: Clean rx and disp globals prior to loading any regimen, make sure we have nothing in state (TFS 133853)
'16Feb16 XN  ProcessPN: Fixed issue with PN supply request not populating Wtranlog.RequestID_Dispensing 145448
'11Jul16 KR  RefreshState: Now check for out-of-use wards and consultants before dispensing.
...
'11Jul16 KR Now check for out-of-use wards and consultants before dispensing.
...
Const ErrSource As String = "RefreshState"
   ...
   Else                                                              'transport object is alive
      If gTransportConnectionIsNothing() Then                        'but not connected      '15Aug12 CKJ
         Set gTransport = Nothing                                    ' terminate the transport object ready for a clean creation below
      Else                                                           'it is connected
         If gTransportConnectionState() = adStateOpen Then           'open & ready for use      '15Aug12 CKJ
            'no action                                               ' no further action, just use it
         Else                                                        'but not properly open
            gTransportConnectionClose                                ' attempt to close (may be connecting, handling an error etc)      '15Aug12 CKJ
            SetgTransportConnectionToNothing                         ' disconnect      '15Aug12 CKJ
            Set gTransport = Nothing                                 ' terminate the transport object ready for a clean creation below
         End If
      End If
...
''   If AscribeSiteNumber = 0 Then Err.Raise 32767, ErrSource, "Invalid AscribeSiteNumber (Zero)"
   ...
   Else
      ...
      If Not gTransport Is Nothing Then
         If Not gTransportConnectionIsNothing() Then      '15Aug12 CKJ
            If gTransportConnectionState() = adStateOpen Then      '15Aug12 CKJ
               'gTransport.ADOSetConnectionTimeOut 5      '15Aug12 CKJ not appropriate
               valid = True
            End If
         End If
      End If
      ...
      If Not valid Then
         ...
         If valid Then
            ...
            phase = 8
            ReadSiteInfo
            ReadSites '26Mar12 TH Added (TFSLogFix)
...
Public Function ProcessPN(ByVal sessionID As Long, _
                           ByVal AscribeSiteNumber As Long, _
                           ByVal PNAction As String, _
                           ByVal RegimenRequestID As Long, _
                           ByVal SupplyRequestID As Long, _
                           ByVal PNPrintXML As String, _
                           ByVal LogLocktoSite As String, _
                           ByVal URLToken As String _
                          ) As Boolean
                            
'16Jan12 TH written, derived from ProcessBatch() from rpt Disp
'PN ACtions
'I   Issue REgimen
'R   Return Regimen
'P   Print for Regimen (wrksheet /lbls)
'C   Combined print and issue (daisy chain I and P)

'14May12 TH added LogLocktoSite (only used with log viewer to determine whether to lock to specific site (TFS 26711)
'11Sep14 XN 88799 Added using or regimen RequestID
'04Nov15 TH Clean rx and disp globals prior to loading any regimen, make sure we have nothing in state (TFS 133853)
'16Feb16 XN ProcessPN: Fixed issue with PN supply request not populating Wtranlog.RequestID_Dispensing 145448
...
Dim strPrevFile As String
Dim intMultiplier As Integer
Dim frmPNProcess As frmPNProcess
Dim intSuccess As Integer

Dim ErrNumber As Long, ErrDescription As String
Const ErrSource As String = "ProcessPN"
   ...
   If Not CancelIsRequested() And Not OCXisBusy() Then
      ...
      If OpenDBConnection(sessionID, AscribeSiteNumber, URLToken) Then
         ...
         On Error GoTo ProcessPNError
         
         ''If gPRNheapID Then Heap 2, gPRNheapID, 0, "", intSuccess
         SetupPRNHeap True, 0     'reset Print heap '20Feb12 TH added to try and prevent caching of heap elements between calls
         
         success = SetPNMode(PNAction)
         
         '04Nov15 TH Clean these prior to loading any regimen, make sure we have nothing in state (TFS 133853)
         gRequestID_Prescription = 0
         gRequestID_Dispensing = 0
         ...
         If success Then
            If PNAction = "E" Then
               PNLayoutEditor False
            ElseIf PNAction = "V" Then
               PNLayoutEditor True
            
            ElseIf PNAction = "L" Then
               'WE need to set a default period and get and set the batch information
               'BatchNumber$ = ""
               'PNLoadRegimen SupplyRequestID 11Sep14 XN 88799
               PNLoadRegimen RegimenRequestID, SupplyRequestID
               '14May12 TH Added (TFS 27611)
               If LogLocktoSite = "Yes" Then
                  frmLogView.txtSite.Enabled = False
               Else
                  frmLogView.txtSite.Enabled = True
               End If
               '------
               
               frmLogView.txtDateFrom.text = Format$(DateAdd("yyyy", -1, Now), "dd mmm yyyy")
               frmLogView.txtTransBatchNumber = GetPNBatchnumber()
               frmLogView.txtLabeltype = "T"
               frmLogView.Show 1, Me		'AS : MS_Edge_Fix for modal windows without an owner form
               Unload frmLogView
            Set frmLogView = Nothing
            Else
            
            SetPNPRintXML PNPrintXML
            SetRegimenRequestID RegimenRequestID    '11Sep14 XN 88799 Added printing of prescription from regimen
            SetSupplyRequestID SupplyRequestID
            
            'setRepeatDispensingAction RepeatDispensingAction
            Set frmPNProcess = New frmPNProcess
            
            Load frmPNProcess
            frmPNProcess.Show 1, Me		'AS : MS_Edge_Fix for modal windows without an owner form
         
            Unload frmPNProcess
            Set frmPNProcess = Nothing
            End If
         End If
      End If
